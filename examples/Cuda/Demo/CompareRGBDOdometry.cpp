...
namespace { // CODES COPIED FROM Core/Odometry/Odometry.cpp
bool CompareFloatImages(Image &src, Image &tgt) {
    /** Edges can be different **/
    for (int y = 2; y < src.height_ - 2; y++) {
        for (int x = 2; x < src.width_ - 2; x++) {
            float *p_src = PointerAt<float>(src, x, y);
            float *p_tgt = PointerAt<float>(tgt, x, y);
            if ((*p_src - *p_tgt) > 1e-4f) {
                PrintError("(%d %d) - (%f %f)\n", x, y, *p_src, *p_tgt);
            }
        }
    }
...
void NormalizeIntensity(Image &image_s, Image &image_t,
    ...
    if (image_s.width_ != image_t.width_ ||
        ...
        PrintError(
            "[NormalizeIntensity] Size of two input images should be same\n");
        return;
    }
...
inline bool CheckRGBDImagePair(const RGBDImage &source,
...
std::tuple<std::shared_ptr<RGBDImage>, std::shared_ptr<RGBDImage>,
    std::shared_ptr<CorrespondenceSetPixelWise>>
InitializeRGBDOdometry(
    ...
    return std::make_tuple(source_out, target_out, correspondence);
}
...
std::tuple<bool, Eigen::Matrix4d> DoSingleIteration(
    ...
    auto f_lambda = [&]
        ...
        jacobian_method.ComputeJacobianAndResidual(i,
                                                   J_r,
                                                   r,
                                                   source,
                                                   target,
...
std::tuple<bool, Eigen::Matrix4d> ComputeMultiscale(
    ...
    for (int level = num_levels - 1; level >= 0; level--) {
        const Eigen::Matrix3d
            level_camera_matrix = pyramid_camera_matrix[level];
...
int TwoFrameRGBDOdometry(
    ...
    /** Settings **/
    OdometryOption option({20, 10, 5}, 0.07);
    Eigen::Matrix4d odo_init = Eigen::Matrix4d::Identity();
    PinholeCameraIntrinsic pinhole_camera_intrinsic =
        PinholeCameraIntrinsic(
            PinholeCameraIntrinsicParameters::PrimeSenseDefault);
    std::shared_ptr<RGBDOdometryJacobian> jacobian_method =
        std::make_shared<RGBDOdometryJacobianFromHybridTerm>();

    /*****************************/
    /** Begin of RGBDOdometryCuda **/
    cuda::RGBDOdometryCuda<3> odometry;
    odometry.SetIntrinsics(pinhole_camera_intrinsic);
    odometry.SetParameters(option, 0.968f);
    odometry.transform_source_to_target_ = odo_init;
    odometry.Initialize(source, target);
    auto correspondences_cuda = odometry.correspondences_.Download();
    odometry.ComputeMultiScale();
    std::cout << odometry.transform_source_to_target_ << std::endl;
    /** End of RGBDOdometryCuda **/
    /*****************************/

    auto source_rgbd = CreateRGBDImageFromColorAndDepth(
        source_color, source_depth);
    auto target_rgbd = CreateRGBDImageFromColorAndDepth(
        target_color, target_depth);

    /** 1. Check depth image truncation & intensity conversion **/
    CompareFloatImages(source_rgbd->depth_, *source.depth_.DownloadImage());
    CompareFloatImages(target_rgbd->depth_, *target.depth_.DownloadImage());
    CompareFloatImages(source_rgbd->color_, *source.intensity_.DownloadImage());
    CompareFloatImages(target_rgbd->color_, *target.intensity_.DownloadImage());

    std::shared_ptr<RGBDImage> source_processed, target_processed;
    std::shared_ptr<CorrespondenceSetPixelWise> correspondences_cpu;
    std::tie(source_processed, target_processed, correspondences_cpu) =
        InitializeRGBDOdometry(*source_rgbd, *target_rgbd,
            pinhole_camera_intrinsic, odo_init, option);

    /** 2. Check depth preprocessing **/
    CompareFloatImages(source_processed->depth_,
        *odometry.source_[0].depth_.DownloadImage());
    CompareFloatImages(target_processed->depth_,
        *odometry.target_[0].depth_.DownloadImage());

    /** 3. Check pyramids **/
    auto source_pyramid = CreateRGBDImagePyramid(*source_processed, 3);
    auto target_pyramid = CreateRGBDImagePyramid(*target_processed, 3);
    auto target_pyramid_dx = FilterRGBDImagePyramid
        (target_pyramid, Image::FilterType::Sobel3Dx);
    auto target_pyramid_dy = FilterRGBDImagePyramid
        (target_pyramid, Image::FilterType::Sobel3Dy);
    ...
    for (int i = 0; i < 3; ++i) {
        CompareFloatImages(*odometry.source_[i].depth_.DownloadImage(),
                           source_pyramid[i]->depth_);
        CompareFloatImages(*odometry.source_[i].intensity_.DownloadImage(),
                           source_pyramid[i]->color_);
        CompareFloatImages(*odometry.target_[i].depth_.DownloadImage(),
                           target_pyramid[i]->depth_);
        CompareFloatImages(*odometry.target_[i].intensity_.DownloadImage(),
                           target_pyramid[i]->color_);
        CompareFloatImages(*odometry.target_dx_[i].depth_.DownloadImage(),
                           target_pyramid_dx[i]->depth_);
        CompareFloatImages(*odometry.target_dy_[i].depth_.DownloadImage(),
                           target_pyramid_dy[i]->depth_);
    }

    /** 4. Cross check initial correspondences **/
    typedef Eigen::Matrix<Eigen::Vector2i, -1, -1> CorrespondenceMap;
    CorrespondenceMap correspondence_map_cuda =
        CorrespondenceMap::Constant(
            source_rgbd->depth_.width_, source_rgbd->depth_.height_,
            Eigen::Vector2i(-1, -1));
    for (int i = 0; i < correspondences_cuda.size(); ++i) {
        auto corres = correspondences_cuda[i];
        correspondence_map_cuda(corres(0), corres(1))
            = Eigen::Vector2i(corres(2), corres(3));
    }
    for (int j = 0; j < (*correspondences_cpu).size(); ++j) {
        auto corres_cpu = (*correspondences_cpu)[j];
        auto corres_cuda = correspondence_map_cuda(corres_cpu(0),corres_cpu(1));
        if (corres_cpu(2) != corres_cuda(0) || corres_cpu(3) != corres_cuda(1)) {
            PrintWarning("Different correspondences: "
                         "(%d %d) -> cpu (%d %d), cuda (%d %d)\n",
                         corres_cpu(0), corres_cpu(1),
                         corres_cpu(2), corres_cpu(3),
                         corres_cuda(0), corres_cuda(1));
        }
    }

    CorrespondenceMap correspondence_map_cpu =
        CorrespondenceMap::Constant(
            source_rgbd->depth_.width_, source_rgbd->depth_.height_,
            Eigen::Vector2i(-1, -1));
    for (int i = 0; i < (*correspondences_cpu).size(); ++i) {
        auto corres = (*correspondences_cpu)[i];
        correspondence_map_cpu(corres(0), corres(1))
            = Eigen::Vector2i(corres(2), corres(3));
    }
    for (int j = 0; j < correspondences_cuda.size(); ++j) {
        auto corres_cuda = correspondences_cuda[j];
        auto corres_cpu = correspondence_map_cpu(corres_cuda(0),corres_cuda(1));
        if (corres_cuda(2) != corres_cpu(0) || corres_cuda(3) != corres_cpu(1)) {
            PrintWarning("Different correspondences: "
                         "(%d %d) -> cpu (%d %d), cuda (%d %d)\n",
                         corres_cuda(0), corres_cuda(1),
                         corres_cpu(0), corres_cpu(1),
                         corres_cuda(2), corres_cuda(3));
        }
    }
    ...
    if (is_success) {
        ...
        std::cout << trans_output << std::endl;
    }
}

int main(int argc, char **argv) {
    std::string base_path = "/media/wei/Data/data/indoor_lidar_rgbd/apartment";
    auto rgbd_filenames = ReadDataAssociation(
        base_path + "/data_association.txt");

    SetVerbosityLevel(VerbosityLevel::VerboseDebug);
    for (int i = 5702; i < 5703; ++i) {
        std::cout << i << std::endl;
