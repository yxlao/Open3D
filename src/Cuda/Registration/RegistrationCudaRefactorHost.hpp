...
/* Preparation */
RegistrationCudaR::RegistrationCudaR(
...
void RegistrationCudaR::Create(
    const registration::TransformationEstimationType &type) {
    type_ = type;
    lambda_geometric_ = 0.968;

    device_ = std::make_shared<RegistrationCudaDeviceR>();
    results_.Create(28);
}

void RegistrationCudaR::Release() {
...
void RegistrationCudaR::UpdateDevice() {
    if (device_ != nullptr) {
...
void RegistrationCudaR::Initialize(
...
/* High-level API */
RegistrationResultCudaR RegistrationCudaR::ComputeICP(int iter) {
    assert(iter > 0);
...
Eigen::Matrix6d RegistrationCudaR::ComputeInformationMatrix() {
    ...
    results_.Memset(0);
    RegistrationCudaKernelCallerR::ComputeInformationMatrix(*this);
...
/* ICP */
RegistrationResultCudaR RegistrationCudaR::DoSingleIteration(int iter) {
    RegistrationResultCudaR result;
...
void RegistrationCudaR::GetCorrespondences() {
#ifdef _OPENMP
#pragma omp parallel
    {
#endif
...
void RegistrationCudaR::ExtractResults(
...
void RegistrationCudaR::ComputeColorGradients(
...
#pragma omp for nowait
    ...
    RegistrationCudaKernelCallerR::ComputeColorGradeint(
        *this, corres_for_color_gradient);
}

RegistrationResultCudaR RegistrationCudaR::Umeyama() {
    RegistrationResultCudaR result;
    ...
    results_.Memset(0);
    RegistrationCudaKernelCallerR::ComputeSumForPointToPointICP(*this);
    std::vector<float> downloaded_result = results_.DownloadAll();
    ...
    std::cout << mean_source.ToEigen() << "\n" << mean_traget.ToEigen() << "\n" << inliers << "\n";

    /** Pass 2: sum reduction Sigma **/
    results_.Memset(0);
    RegistrationCudaKernelCallerR::BuildLinearSystemForPointToPointICP(
