...
void MarchingCubesVertexAllocationKernel(
    ...
    uchar &table_index = mesher.table_indices(x, y, z);
    table_index = 0;
...
#pragma unroll 12
    for (int i = 0; i < 12; ++i) {
        if (edges & (1 << i)) {
            mesher.vertex_indices(x + edge_shift[i][0],
                                  y + edge_shift[i][1],
                                  z + edge_shift[i][2])
                (edge_shift[i][3]) = 0;
        }
...
void MarchingCubesVertexExtractionKernel(
    ...
    Vector3i &vertex_index = mesher.vertex_indices(x, y, z);
    ...
    if (f1) {
        ...
        vertex_index(0) = mesher.mesh().vertices().push_back(
            server.voxel_to_world(x + mu, y, z));

        if (type & VertexWithNormal) {
            mesher.mesh().vertex_normals()[vertex_index(0)] =
            server.transform_volume_to_world_.Rotate(
                (1 - mu) * server.gradient(x, y, z)
                    + mu * server.gradient(x + 1, y, z));
        }
    }
    ...
    if (f2) {
        ...
        vertex_index(1) = mesher.mesh().vertices().push_back(
            server.voxel_to_world(x, y + mu, z));

        if (type & VertexWithNormal) {
            mesher.mesh().vertex_normals()[vertex_index(1)] =
            server.transform_volume_to_world_.Rotate(
                (1 - mu) * server.gradient(x, y, z)
                    + mu * server.gradient(x, y + 1, z));
        }
    }
    if (f3) {
        float tsdf4 = server.tsdf(x, y, z + 1);
        float mu = (0 - tsdf0) / (tsdf4 - tsdf0);
        vertex_index(2) = mesher.mesh().vertices().push_back(
            server.voxel_to_world(x, y, z + mu));

        if (type & VertexWithNormal) {
            mesher.mesh().vertex_normals()[vertex_index(2)] =
            server.transform_volume_to_world_.Rotate(
                (1 - mu) * server.gradient(x, y, z)
                    + mu * server.gradient(x, y, z + 1));
        }
    }
...
void MarchingCubesTriangleExtractionKernel(
     ...
     * save it **/
    const uchar table_index = mesher.table_indices(x, y, z);
