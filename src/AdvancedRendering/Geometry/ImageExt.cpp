...
std::shared_ptr<Image> FlipImageExt(const Image &input) {
    ...
    output->PrepareImage(input.width_, input.height_, input.num_of_channels_,
                         input.bytes_per_channel_);

    const int stride =
            input.width_ * input.num_of_channels_ * input.bytes_per_channel_;
    for (int y = 0; y < input.height_; y++) {
        auto input_ptr = input.data_.data() + y * stride;
        auto output_ptr =
                output->data_.data() + (input.height_ - 1 - y) * stride;
...
namespace io {
bool WriteImageToHDR(const std::string &filename,
    ...
    stbi_write_hdr(filename.c_str(), image.width_, image.height_,
                   image.num_of_channels_, (float *)image.data_.data());
    return true;
}
