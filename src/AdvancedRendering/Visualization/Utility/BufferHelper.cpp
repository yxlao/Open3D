...
bool BindTexture2D(GLuint &texture_id,
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, format, texture.width_, texture.height_, 0,
                 format, type, texture.data_.data());
...
GLuint CreateTexture2D(GLuint width,
                       GLuint height,
...
std::shared_ptr<geometry::Image> ReadTexture2D(GLuint width,
                                               GLuint height,
                                               int channels,
...
GLuint BindTextureCubemap(const std::vector<geometry::Image> &textures,
    ...
    for (int i = 0; i < 6; ++i) {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB16F,
                     textures[i].width_, textures[i].height_, 0, GL_RGB,
                     GL_FLOAT, textures[i].data_.data());
    }
...
GLuint CreateTextureCubemap(GLuint size,
                            bool use_mipmap,
                            const visualization::RenderOption &option) {
    GLuint texture_id;
    glGenTextures(1, &texture_id);
...
void LoadViews(std::vector<GLHelper::GLMatrix4f> &views) {
    views = {
            GLHelper::LookAt(Eigen::Vector3d::Zero(), Eigen::Vector3d(+1, 0, 0),
                             Eigen::Vector3d(0, -1, 0)),
            GLHelper::LookAt(Eigen::Vector3d::Zero(), Eigen::Vector3d(-1, 0, 0),
                             Eigen::Vector3d(0, -1, 0)),
            GLHelper::LookAt(Eigen::Vector3d::Zero(), Eigen::Vector3d(0, +1, 0),
                             Eigen::Vector3d(0, 0, +1)),
            GLHelper::LookAt(Eigen::Vector3d::Zero(), Eigen::Vector3d(0, -1, 0),
                             Eigen::Vector3d(0, 0, -1)),
            GLHelper::LookAt(Eigen::Vector3d::Zero(), Eigen::Vector3d(0, 0, +1),
                             Eigen::Vector3d(0, -1, 0)),
            GLHelper::LookAt(Eigen::Vector3d::Zero(), Eigen::Vector3d(0, 0, -1),
                             Eigen::Vector3d(0, -1, 0))};
}
...
bool CheckGLState(const std::string &msg) {
    ...
    if (ret != GL_NO_ERROR) {
        utility::PrintWarning("[OpenGL error]: %d at %s\n", ret, msg.c_str());
        return false;
    }
    return true;
}
